...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const resOnAvater = await uploadOnCloudinary(avatarLocalPath);
  const resOnCoverImage = await uploadOnCloudinary(coverImageLocalPath);
  ...
  const user = await User.create({
    fullName,
    avater: resOnAvater.url,
    coverImage: resOnCoverImage?.url || "",
    email,
    password,
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    throw new ApiResponse(201, error?.message || "Invalid refresh token");
  }
});
...
const updateUserAvater = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, user, "avater Image updated successfully"));
});
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!coverImage.url) {
    throw new ApiError(400, "Error uploading on cover image");
  }
...
export {
  ...
  getCurrentUser, getCurrentUser, getUserChannelProfile,
